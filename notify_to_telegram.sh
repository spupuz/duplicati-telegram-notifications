...
function parseResultFile () {
    ...
    while IFS=':' read -r key val || [ -n "$key" ]; do
        ...
        if [[ "$key" =~ ^[a-zA-Z_][a-zA-Z0-9_]*$ ]]; then
            # Escape HTML to prevent Telegram API errors (XSS/HTML injection)
            val=${val//&/"&amp;"}
            val=${val//</"&lt;"}
            val=${val//>/"&gt;"}
...
# Function to safely HTML-escape user inputs (prevents HTML injection leading to API 400 errors)
