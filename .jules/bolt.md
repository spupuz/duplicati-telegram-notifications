...
## 2024-10-25 - Removed runtime multiline string trimming loops
**Learning:** While native bash multiline string trimming loops are faster than subshells (e.g. `sed`), they are still computationally expensive when executed repeatedly for string generation. We discovered that formatting the multiline strings correctly from the start completely avoids the need for this runtime formatting loop, allowing direct output passage and leading to massive (10x+) speed improvements for function execution.
**Action:** Statically format your bash multiline strings appropriately to bypass any need for dynamic trimming loops during runtime execution.
