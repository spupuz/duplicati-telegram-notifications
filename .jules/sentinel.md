...
## 2024-05-28 - Prevent Telegram API 400 Bad Request via HTML Injection
**Vulnerability:** When sending a message to Telegram using `parse_mode="HTML"`, any unescaped HTML characters (`<`, `>`, `&`) in the user-controlled message content would cause the API to return a 400 Bad Request. Since the script silently ignores `curl` errors (e.g. `> /dev/null`), a backup failure containing unescaped characters (like `<path> not found`) would silently drop the notification, leading to a Denial of Service (DoS) for alerts.
**Learning:** Telegram's HTML parsing is strict and requires proper escaping. Escaping must be performed using backslashes to avoid string pattern substitution bugs in bash (e.g., `val="${val//&/\&amp;}"`). Furthermore, escaping `&` must happen *before* escaping `<` and `>` to avoid double-escaping.
**Prevention:** Implemented an `escape_html_var` function using native bash substitution and applied it to user-controlled variables (like backup name, operation name, event name, parsed result, error details) before formatting the Telegram HTML payload.
