...
## 2026-08-25 - [Fix HTML Injection DoS in Telegram Payload]
**Vulnerability:** User-controlled environment variables (e.g. backup name) and parsed result fields (like error messages) could contain unescaped HTML characters (`<`, `>`, `&`). Since the Telegram payload uses HTML format (`parse_mode="HTML"`), an unescaped tag like `<Malicious>` causes the API to reject the payload with a 400 Bad Request error, leading to silent notification failures (DoS).
**Learning:** External variables (such as those dynamically populated by Duplicati) passed to external services that parse specific markup (like Telegram HTML or Markdown) are vulnerable to injection attacks that disrupt the notification pipeline. The escaping must specifically handle `&` first to avoid double escaping.
**Prevention:** Always HTML encode user-controlled and dynamically loaded string variables before inserting them into any formatted layout that requires strict parsing rules.
