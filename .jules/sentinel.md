...
## 2026-08-21 - HTML Injection (DoS) in Telegram Notification Payload
**Vulnerability:** The `notify_to_telegram.sh` script utilized Telegram's `parse_mode="HTML"` but directly injected dynamic variables (such as `$DUPLICATI__backup_name`, `$RES_Failed`, and `$RES_Details`) into the message string without escaping them. An attacker who could influence these variables (e.g., via backup name or log file errors) could inject unescaped characters like `<`, `>`, and `&`. This resulted in malformed HTML being sent to the Telegram API, which returns a `400 Bad Request` and silently fails to send the notification (Denial of Service).
**Learning:** When generating payloads intended for a parser (like an HTML rendering API), all externally controlled inputs and strings must be encoded/escaped to match the target context. Failing to do so allows injection attacks that can break functionality or execute code if interpreted by a browser.
**Prevention:** Sanitize all variables before rendering them in an HTML template. For Bash, this can be done securely and efficiently using parameter expansion in a reusable assignment function (e.g., replacing `&`, `<`, and `>`).
