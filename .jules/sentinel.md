...
## 2024-05-28 - [HTML Injection in Telegram Notifications]
**Vulnerability:** The script used `parse_mode="HTML"` to send Telegram notifications but didn't sanitize user-provided details (like backup names or error details). This meant that backup names or log outputs containing `<` or `&` characters caused Telegram to reject the HTTP request (400 Bad Request) due to invalid HTML, resulting in silent notification failures (Denial of Service).
**Learning:** Even internal tool notifications can be fragile if they assume all input is safe. An unescaped ampersand in a file path could cause critical backup failures to go unnoticed.
**Prevention:** Always escape HTML entities (`<`, `>`, `&`) in any variables before inserting them into an HTML-formatted message payload, even when those variables come from local files or environment properties.
