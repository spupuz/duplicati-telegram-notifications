...
## 2026-09-26 - Prevent Secret Leakage via Process List (Command-Line Arguments)
**Vulnerability:** Information Disclosure (CWE-200) occurred because the Telegram API URL, containing the secret `TELEGRAM_TOKEN`, was passed directly as a command-line argument to the `curl` command. Even if the variable is un-exported and hidden from child process environments, command-line arguments of running processes are globally visible to other users on the system via the process list (e.g., `ps aux` or reading `/proc/<pid>/cmdline`).
**Learning:** System process lists expose command-line arguments to all users. Passing sensitive tokens, passwords, or keys as arguments to external commands (like `curl`, `mysql`, or `wget`) makes them vulnerable to exposure during execution.
**Prevention:** Always pass sensitive data to external tools via secure channels that are not visible in the process list. For `curl`, use configuration files or pipe the configuration via standard input (`-K -` or `--config -`) to hide URLs containing secrets.
