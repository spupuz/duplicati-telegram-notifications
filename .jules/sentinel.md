...
## 2024-05-24 - [DoS via Telegram HTML Parsing]
**Vulnerability:** The `notify_to_telegram.sh` script utilized `parse_mode="HTML"` in the Telegram API call without escaping user-controlled variables (e.g. `$DUPLICATI__backup_name`, `$RES_Failed`). Unescaped characters like `<` and `&` in these variables would result in a malformed HTML payload, causing the Telegram API to return a 400 Bad Request error and dropping the notification (DoS vulnerability).
**Learning:** Even internal tool notifications can be vulnerable to DoS if they pass unescaped user-controlled input into strict parsers (like Telegram's HTML mode).
**Prevention:** Always escape HTML characters (`<`, `>`, `&`) using native bash parameter expansion (e.g., `val=${val//</"&lt;"}`) for any user-controllable input before integrating it into a payload intended for an HTML parser.
